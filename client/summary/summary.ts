...
export class Summary {
  ...
  list() {
    ...
    this.trackItemService
      .findAllDayItems(this.searchMinDate, this.searchMaxDate, 'LogTrackItem')
      .then(items => {
        ...
        let itemDiffs = items.map(item =>
          moment(item.endDate).diff(item.beginDate),
        );
  ...
  deleteTrackItems() {
    let selectedIds = this.trackItems
      .filter(item => item.$isSelected === true)
      .map(item => item.id);
    console.log(selectedIds);

    if (selectedIds.length > 0) {
      this.trackItemService.deleteByIds(selectedIds).then(item => {
        logger.debug('Deleted trackitems from DB:', selectedIds);
        this.list();
      });
    } else {
      logger.debug('No ids, not deleting from DB');
    }
  }
}
