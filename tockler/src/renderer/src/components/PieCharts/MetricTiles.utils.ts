import { minDayjs, maxDayjs } from '@renderer/dayjs.config';
import { convertDate } from '../../constants';

export const clampRange = (clampToRange, range) => {
    const begin = maxDayjs(clampToRange[0], convertDate(range[0]));
    const end = minDayjs(clampToRange[1], convertDate(range[1]));
    return [begin, end];
};
