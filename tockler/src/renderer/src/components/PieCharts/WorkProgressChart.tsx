...
export const WorkProgressChart = ({ items, width, hoursToWork }) => {
    ...
    if (pieData.length) {
        const workDay = dayjs.duration(Number(hoursToWork), 'hours');

        const workedMs = pieData[0].timeDiffInMs;

        // pauses must be 10% of worked time
        const pausesInHour = 0.1;
        const worked = dayjs.duration(workedMs);
        const pauses = dayjs.duration(workedMs * pausesInHour);
    ...
    return (
        <VictoryPie
            ...
            style={style}
            labels={({ datum }) => {
                const dur = dayjs.duration(datum.timeDiffInMs);
