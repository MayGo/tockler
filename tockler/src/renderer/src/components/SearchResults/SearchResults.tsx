...
const ActionCell = ({ cell }) => {
    ...
    const goToTimelinePage = (record) => {
        loadTimerange([dayjs(record.beginDate).startOf('day'), dayjs(record.beginDate).endOf('day')]);
        setVisibleTimerange([
            dayjs(record.beginDate).subtract(15, 'minutes'),
            dayjs(record.endDate).add(15, 'minutes'),
        ]);
        navigate('/timeline');
    };
