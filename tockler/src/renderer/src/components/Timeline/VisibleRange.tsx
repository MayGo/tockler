...
export const VisibleRange = memo(() => {
    ...
    const showDay = () => {
        const beginDate = dayjs(timerange[0]).startOf('day');
        const endDate = dayjs(timerange[0]).endOf('day');
        setVisibleTimerange([beginDate, endDate]);
    };

    const showHour = () => {
        const beginDate = dayjs(visibleTimerange[1]).startOf('hour');
        const endDate = dayjs(visibleTimerange[1]).endOf('hour');
        setVisibleTimerange([beginDate, endDate]);
    };

    const showAM = () => {
        const beginDate = dayjs(timerange[0]).startOf('day');
        const endDate = dayjs(timerange[0]).startOf('day').hour(12);
        setVisibleTimerange([beginDate, endDate]);
    };

    const showPM = () => {
        const beginDate = dayjs(timerange[0]).startOf('day').hour(12);
        const endDate = dayjs(timerange[0]).endOf('day');
        setVisibleTimerange([beginDate, endDate]);
    };

    const showEvening = () => {
        const beginDate = dayjs(visibleTimerange[0]).startOf('day').hour(17);
        const endDate = dayjs(visibleTimerange[0]).endOf('day');
        setVisibleTimerange([beginDate, endDate]);
    };
