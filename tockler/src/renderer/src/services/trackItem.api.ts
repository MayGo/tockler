...
async function findAllDayItems(from: Dayjs, to: Dayjs, taskName: string): Promise<any> {
...
export async function findAllDayItemsForEveryTrack(from: Dayjs, to: Dayjs) {
...
export function startNewLogItem(oldItem: any) {
    ...
    newItem.beginDate = dayjs().valueOf();
    newItem.endDate = dayjs().add(60, 'seconds').valueOf();
